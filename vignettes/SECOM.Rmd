...
## 4.3 Alternative input formats

The calls in Section 4.1 also accept `TreeSummarizedExperiment` objects or
count matrices. Only the data arguments differ; all other arguments are
unchanged.

For a `TreeSummarizedExperiment` object, pass `data = list(tse)` and
`assay_name = "counts"`.
...
For count matrices, three objects are supplied: the abundance data at the
...
`data = list(abundance_data)`, `aggregate_data = list(aggregate_data)`, and
`meta_data = list(meta_data)`, and drop `tax_level`.
...
Correlations within and across ecosystems are computed on the samples that the
ecosystems have in common, so the sample names must overlap.
...
## 5.4 Alternative input formats

The calls in Section 5.2 also accept `TreeSummarizedExperiment` objects or
count matrices. Only the data arguments differ; all other arguments are
unchanged.

For `TreeSummarizedExperiment` objects, pass `data = list(CE = tse1, NE = tse2)`
and `assay_name = c("counts", "counts")`.
...
For count matrices, pass
`data = list(CE = abundance_data1, NE = abundance_data2)`,
`aggregate_data = list(aggregate_data1, aggregate_data2)`, and
`meta_data = list(meta_data1, meta_data2)`, and drop `tax_level`.
