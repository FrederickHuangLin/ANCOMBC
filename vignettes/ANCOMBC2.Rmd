...
pseudo-counts on zero counts for each taxon. This involves adding various 
pseudo-count values (0.1, 0.5, and 1) to the feature table 
and re-running the ANCOM-BC2 algorithm. The sensitivity score for each taxon is 
...
results obtained without adding pseudo-counts to zero counts (using the complete 
data), then the taxon is considered not sensitive to the pseudo-count addition. 
**Important:** The sensitivity analysis results depend directly on your chosen 
alpha value (since this threshold determines significance). You should expect 
sensitivity analysis results to change when you modify the alpha value.
...
## 3.4 Sensitivity analysis with `conservative = FALSE`

The `conservative` argument specifies at which stage the pseudo-counts are
added. The default, `conservative = TRUE`, adds the pseudo-counts 0.1, 0.5, and
1 to the feature table before the estimation of the sampling fractions and
re-runs the entire ANCOM-BC2 algorithm for each value. With
`conservative = FALSE`, the sampling fractions are estimated once on the
complete data, the pseudo-counts `seq(0.01, 0.5, 0.01)` are added to the zero
counts of the bias-corrected data, and only the inference step is re-run for
each value. The `passed_ss` and `diff_robust` columns are reported under both
settings.
...
The two settings flag the same taxa as robust in the pairwise directional test
on this dataset: 7, 7, and 12 taxa for the three comparisons. They differ in
the sensitivity analysis itself. Under `conservative = FALSE`, 72, 70, and 90
of the 91 taxa are declared insensitive to the pseudo-count addition, compared
with 67, 55, and 84 taxa under the default. The taxa whose sensitivity flags
differ between the two settings are not significant in the pairwise directional
test.
...
passage through the sensitivity analysis (`diff_robust` is `TRUE`) for 
